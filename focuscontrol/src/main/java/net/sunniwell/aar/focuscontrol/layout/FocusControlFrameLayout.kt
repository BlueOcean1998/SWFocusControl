 ...
 */
open class FocusControlFrameLayout @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    @AttrRes defStyleAttr: Int = 0,
    @StyleRes defStyleRes: Int = R.style.FocusControlFrameLayout
) : FrameLayout(context, attrs, defStyleAttr), FocusControlViewParent {
    companion object {
    ...
    init {
        val typedArray = context.obtainStyledAttributes(
            attrs, R.styleable.FocusControlFrameLayout, 0, defStyleRes
        )
        val recordFocusEnabled = typedArray.getBoolean(
            R.styleable.FocusControlFrameLayout_recordFocusEnabled, false
        )
        val defFocusId = typedArray.getResourceId(
            R.styleable.FocusControlFrameLayout_defFocusId, NO_ID
        )
        /*获取searchFocus属性值，如果获取不到resourceId则获取int值*/
        val searchFocusLeft = typedArray.getResourceId(
            R.styleable.FocusControlFrameLayout_searchFocusLeft,
            typedArray.getInt(
                R.styleable.FocusControlFrameLayout_searchFocusLeft, DEF_SEARCH_FOCUS
            )
        )
        val searchFocusRight = typedArray.getResourceId(
            R.styleable.FocusControlFrameLayout_searchFocusRight,
            typedArray.getInt(
                R.styleable.FocusControlFrameLayout_searchFocusRight, DEF_SEARCH_FOCUS
            )
        )
        val searchFocusUp = typedArray.getResourceId(
            R.styleable.FocusControlFrameLayout_searchFocusUp,
            typedArray.getInt(
                R.styleable.FocusControlFrameLayout_searchFocusUp, DEF_SEARCH_FOCUS
            )
        )
        val searchFocusDown = typedArray.getResourceId(
            R.styleable.FocusControlFrameLayout_searchFocusDown,
            typedArray.getInt(
                R.styleable.FocusControlFrameLayout_searchFocusDown, DEF_SEARCH_FOCUS
            )
        )
        typedArray.recycle()
