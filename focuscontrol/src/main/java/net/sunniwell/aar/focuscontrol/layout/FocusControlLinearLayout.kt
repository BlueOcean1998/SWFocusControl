 ...
 */
open class FocusControlLinearLayout @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    @AttrRes defStyleAttr: Int = 0,
    @StyleRes defStyleRes: Int = R.style.FocusControlLinearLayout
) : LinearLayout(context, attrs, defStyleAttr), FocusControlViewParent {
    companion object {
    ...
    init {
        val typedArray = context.obtainStyledAttributes(
            attrs, R.styleable.FocusControlLinearLayout, 0, defStyleRes
        )
        val recordFocusEnabled = typedArray.getBoolean(
            R.styleable.FocusControlLinearLayout_recordFocusEnabled, false
        )
        ...
        val searchFocusLeft = typedArray.getResourceId(
            R.styleable.FocusControlLinearLayout_searchFocusLeft,
            typedArray.getInt(
                R.styleable.FocusControlLinearLayout_searchFocusLeft, DEF_SEARCH_FOCUS
            )
        )
        val searchFocusRight = typedArray.getResourceId(
            R.styleable.FocusControlLinearLayout_searchFocusRight,
            typedArray.getInt(
                R.styleable.FocusControlLinearLayout_searchFocusRight, DEF_SEARCH_FOCUS
            )
        )
        val searchFocusUp = typedArray.getResourceId(
            R.styleable.FocusControlLinearLayout_searchFocusUp,
            typedArray.getInt(
                R.styleable.FocusControlLinearLayout_searchFocusUp, DEF_SEARCH_FOCUS
            )
        )
        val searchFocusDown = typedArray.getResourceId(
            R.styleable.FocusControlLinearLayout_searchFocusDown,
            typedArray.getInt(
                R.styleable.FocusControlLinearLayout_searchFocusDown, DEF_SEARCH_FOCUS
            )
        )
        typedArray.recycle()
